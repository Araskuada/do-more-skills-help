...
因此，RQ2 的当前答案是：**与 query 共享关键词、或者会被当前 retriever 自己排到前面的 hard negatives 最容易让 agent 选错 skill。**
...
因此，用 gold skill metadata 构造 same-category 或 same-repo distractors 会不公平，也不可复现。正式 RQ2 改用四类可由当前数据稳定构造的 distractor：
...
- `query_overlap`、`bm25_hard`、`gold_skill_near`：确定性 hard-negative 构造，每个 pool size 运行 1 次。
...
- `query_overlap` 和 `gold_skill_near` 使用 token Jaccard，是 lexical proxy，不是真正 embedding semantic similarity。
...
2. **Semantic hard negatives**：使用 local embedding model 或官方 search server semantic index 构造真正的 semantic-near distractors。
3. **Manual error taxonomy**：抽样标注 query-overlap 和 BM25-hard 的错误案例，区分 broad keyword collision、tool-domain collision、task-format collision 等错误来源。
